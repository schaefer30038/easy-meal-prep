...
//		assertFalse(testFood.createFood("Test Food", "Test Description", null));
	...
	void testSearchFood() {
		...
		try {
			...
			assertTrue(testFood.deleteFood("Testing1"));
			assertTrue(testFood.deleteFood("Test Food"));
	...
	void testUpdateFood() {
		...
		testFood = new Food(connect.getConnection(), account.getUserAccount());
		assertTrue(testFood.createFood("Test Food", "Description", null));

		ResultSet rs = testFood.listAllFood();
		assert (rs != null);
		int foodID = 0;
		try {
			while (rs.next()) {
				if (rs.getString("foodName").equals("Test Food"))
				{
					if (rs.getString("userAccount").equals("Testing"))
					{
		...
		assertTrue(testFood.updateFood(foodID, "Test Food1", "Test Description", null));
		...
		testFood = new Food(connect.getConnection(), account.getUserAccount());
		assertTrue(testFood.deleteFood("Test Food1"));
